...
namespace DG_Laser
{
    ...
    class Laser_Operation
    {
        ...
        public void Read(string Address,string CC)//读取数据没有D1-Dn
        {
            ...
            //发送数据
            Program.SystemContainer.Laser_Control_Com.Send_Data(CC_Data.Sum);
            //等待数据读取完成
            Thread.Sleep(200);
        }
        //写入数据
        ...
        {
            byte[] Rec_Data = null;
            Rec_Data = new byte[Program.SystemContainer.Laser_Control_Com.Receive_Byte.Length];
            Rec_Data = (byte[])Program.SystemContainer.Laser_Control_Com.Receive_Byte.Clone();
            //清空数据
            Resolve_Rec.Empty();
            //数据拆分
